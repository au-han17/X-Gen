...
class CaptionModel(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        #The kwargs.get('mode', 'forward') function attempts to get the value of the 'mode' key from the kwargs dictionary. If the 'mode' key is not present in the dictionary, it will return the default value 'forward'.
        if 'mode' in kwargs:
            del kwargs['mode']
        return getattr(self, '_' + mode)(*args, **kwargs)
    #a wrapper for calling different class methods depending on the mode argument
    #The getattr function will return a reference to the specified method, which is then called with the provided *args and **kwargs as arguments.
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            _tmp_beam_logprobs = unaug_logprobs[state_ix].reshape(batch_size, -1, vocab_size) #(16,3,1944)
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    state_table[divm] = beam_step(logprobs,unaug_logprobs,bdash,t - divm,beam_seq_table[divm],beam_seq_logprobs_table[divm],beam_logprobs_sum_table[divm],state_table[divm])
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), cap=cap,*(
