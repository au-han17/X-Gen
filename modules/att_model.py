...
from .tokenizers import modeBOS,modeEOS,modePAD
...
class AttModel(CaptionModel):
    def __init__(self, args, tokenizer):
        ...
        self.bos_idx = modeBOS
        self.eos_idx = modeEOS
        self.pad_idx = modePAD
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1,*,cap):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks,cap=cap)
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={},*,cap):
        ...
        logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state,cap=cap)

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks = utils.repeat_tensors(beam_size,[p_fc_feats, p_att_feats,pp_att_feats, p_att_masks])
        
        self.done_beams = self.beam_search(state, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, cap=cap,opt=opt)
    ...
    def _sample(self, fc_feats, att_feats, att_masks=None,*,cap):
        opt = self.args.__dict__
        #__dict__ is a special attribute that returns a dictionary representation of an object's attributes and their values.The __dict__ attribute of this object is used to convert the parsed arguments into a dictionary, where the keys represent the argument names and the values represent the values provided to those arguments.
        sample_method = opt.get('sample_method', 'greedy')
        # get is a built-in method for dictionaries in python. if return none,default is greedy 
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(fc_feats, att_feats, att_masks, opt,cap=cap)
        if group_size > 1:
        ...
        for t in range(self.max_seq_length + 1):
            ...
            logprobs, state = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, state,output_logsoftmax=output_logsoftmax,cap=cap)
#logprobs (4,1944)
